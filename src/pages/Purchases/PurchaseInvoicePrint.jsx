// src/components/Purchases/PurchaseInvoicePrint.jsx
...
import { getPurchaseInvoiceByRef } from "./PurchaseService"; // adjust path as needed

const toNum = (v) => (v === "" || v === null || v === undefined ? 0 : Number(v));

const PurchaseInvoicePrint = ({ referenceNo, onClose }) => {
    ...
    const handlePdf = () => {
        const opt = {
            margin: 0.3,
            filename: `Purchase_${referenceNo}.pdf`,
                        ...
                        {items.map((it, i) => (
                            <tr key={i}>
                                ...
                                <td className="border px-2 py-1 text-right">
                                    {toNum(it.purchaseQuantity || it.quantity)}
                                </td>
                                <td className="border px-2 py-1 text-right">
                                    ₹ {toNum(it.unitPurchasePrice || it.unitPrice || 0).toFixed(2)}
                                </td>
                                <td className="border px-2 py-1 text-right">
